...
class NumberInputDialog extends StatefulWidget {
  NumberInputDialog(
      ...
      Key key})
      : assert(value != null),
        assert(minValue == null || minValue <= value, 'value must be equal or greater than minValue'),
        assert(maxValue == null || maxValue >= value, 'value must be equal or lower than maxValue'),
        assert(minValue == null || maxValue == null || maxValue > minValue, 'maxValue must be greater than minValue'),
        super(key: key);
  ...
  static Future<ValueDialogResult<num>> show(BuildContext context, String titleText, {num value, num minValue}) async {
      ...
      inputFormatters: [
        TextInputFormatter.withFunction((oldValue, newValue) {
          if (newValue.text.isNullOrEmpty()) {
            return oldValue.copyWith(selection: TextSelection(baseOffset: 0, extentOffset: oldValue.text.length));
          }
          final newNumValue = num.tryParse(newValue.text);
          if (newNumValue != null) {
            return newValue.copyWith(text: _getBoundedValue(newNumValue).toString());
          }
          return oldValue;
        })
      ],
