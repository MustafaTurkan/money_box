    ...
    if (widget.animation) {
      ...
      _animation = Tween<double>(begin: 0, end: widget.percent).animate(
        CurvedAnimation(parent: _animationController, curve: widget.curve),
          ...
          if (widget.restartAnimation && _percent == 1) {
                ...
                child: Transform.rotate(
                  angle: radians(
                      (widget.circularStrokeCap != CircularStrokeCap.butt &&
                          ...
                          : 0).toDouble(),
                  child: Transform.rotate(
                    angle: radians((widget.reverse ? -360 : 360) * _percent).toDouble(),
    ...
    if (linearGradient != null) {
      if (rotateLinearGradient && progress > 0) {
        ...
        _paintLine.shader = SweepGradient(
                transform: reverse
                    ? GradientRotation(
                        radians(-90 - progress + startAngle).toDouble() - correction)
                    : GradientRotation(
                        radians(-90.0 + startAngle).toDouble() - correction),
