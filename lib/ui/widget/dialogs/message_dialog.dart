...
import 'package:money_box/ui/widget/space.dart';
...
List<Widget> _createDialogButtons(
  ...
  return [
    ...
    if (buttons.index == DialogButton.yesNo.index || buttons.index == DialogButton.yesNoCancel.index)
      TextButton(
          style: TextButton.styleFrom(primary: textColor),
          onPressed: () {
    ...
    var result = await showDialog<DialogResult>(
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: SingleChildScrollView(child: Center(child: Text(message ?? ''))),
...
class MessageSheet {
  static Future<DialogResult> error({
    ...
    final appTheme = context.getTheme();
    return _show(context, title ?? Localizer.of(context).error, message,
  ...
  static Future<DialogResult> info({
    ...
    return _show(context, title ?? context.getLocalizer().information, message,
        buttons: buttons, color: appTheme.colors.info, icon: AppIcons.informationOutline);
  }
  ...
  static Future<DialogResult> question({
    ...
    return _show(context, title ?? context.getLocalizer().question, message,
        buttons: buttons, color: appTheme.colors.info, icon: AppIcons.helpCircleOutline);
  }
  ...
  static Future<DialogResult> message({
    ...
    return _show(context, title ?? context.getLocalizer().message, message,
        buttons: buttons, color: appTheme.colors.primary, icon: AppIcons.messageOutline);
  }
        ...
        return Column(
          ...
          children: <Widget>[
            Container(
              ...
              color: color,
              padding: const EdgeInsets.symmetric(horizontal: Space.l, vertical: Space.l),
              child: Column(
                          ...
                          ? Row(
                              ...
                              children: <Widget>[
                                Expanded(
                                  child: Text(
                                    message,
                                    style: appTheme.textStyles.bodyBold.copyWith(color: Colors.white),
                                  ),
                                ),
                                SizedBox(
                                  width: 16,
