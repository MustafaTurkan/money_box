...
abstract class TextFieldValidator extends FieldValidator<String> {
  ...
  String call(String value) {
    return (ignoreEmptyValues && value.isNullOrEmpty()) ? null : super.call(value);
  }
...
class MaxLengthValidator extends TextFieldValidator {
  MaxLengthValidator({@required this.max, @required String errorText}) : super(errorText);
...
class MinLengthValidator extends TextFieldValidator {
  MinLengthValidator({@required this.min, @required String errorText}) : super(errorText);
...
class PatternValidator extends TextFieldValidator {
  PatternValidator({@required this.pattern, @required String errorText}) : super(errorText);
...
class DateTextValidator extends TextFieldValidator {
  DateTextValidator({@required this.format, @required String errorText}) : super(errorText);
...
class RequiredValidator<T> extends FieldValidator<T> {
  ...
  bool isValid(T value) {
    if (value is String) {
      ...
      return !value.isNullOrEmpty();
    } else if (value is CodeDescription) {
      return value != null && !value.code.isNullOrEmpty();
    }
    return value != null;
  }
}
...
class MatchValidator extends TextFieldValidator {
  MatchValidator({@required this.matchValue, @required String errorText}) : super(errorText);
